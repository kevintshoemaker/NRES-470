...
$g(V,P)$ is known as the **numerical response** -- that is, the increase in per-capita predator population growth due to prey consumption, usually modeled as as a function of prey density, but sometimes modeled as a function of both predator and prey densities.
...
Under such circumstances, the predators do NOT interfere with one another. Each predator consumes a fixed number of prey in a given time period, depending on prey availability. 

Consider the case where there are $V$ prey and only a single predator.  

**Q**: Imagine there are 100 prey and only one predator. A total of 3 prey are consumed in a single day. What is the daily rate of prey consumption in this example ($\alpha$)? 
...
**Q** Can *alpha* go above 1? [this one is tricky]
