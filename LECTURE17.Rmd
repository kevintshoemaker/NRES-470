...
![](predation1.jpg){width=40%}

![](predation2.jpg){width=40%}
...
$f(V)$ is known as the **functional response** -- that is, the predicted per-capita rate of prey consumption as a function of prey density [sometimes also a function of both predator and prey densities ($f(V,P)$)].
...
In the absence of predators, let's assume that the prey population grows according to the basic exponential growth equation:
...
Under such circumstances, the predators do NOT interfere with one another and predators never get full. Each predator consumes a fixed number of prey in a given time period, depending only on prey availability. 

Imagine there are 100 prey and only one predator. A total of 3 prey are consumed in a single day. What is the daily rate of prey consumption per predator in this example ($\alpha$)? 

In this case, 3% (0.03) of prey are successfully encountered and consumed by the predator each day. If there were 1000 prey on the landscape, this single predator would catch and consume 30 prey. And if there were 1 million prey, the predator would catch and consume 30,000 prey per day. (realistic?) 
...
**Step 4**: In the settings menu, change the simulation length to 100 and make sure that your analysis algorithm is set to "4th order runge-kutta" (this makes InsightMaker behave more like a continuous growth model).

**Step 5**: Set the Initial abundance of prey and predators to 15 and 20, respectively. Set the alpha and beta terms to 0.01, and set the r_prey and q_pred terms to 0.15. Run the model and visualize the time series for prey and predators.

**Q** What happens if you change the initial abundances?

**Q** What happens if you change initial abundance of both species to 15? Is this an **equilibrium**? If so, is it **stable**?

**Step 6**: Try some other parameter values for alpha, beta, r, and q... [Top Hat]
...
You can find a fully worked InsightMaker model [here](https://insightmaker.com/insight/2GFgm0holBRAfw47eJ9Xup/Worked-prey-predator-model)
