...
We will do a review session in class on Monday April 11.  
...
Broadly speaking, a **metapopulation** is a group of populations (called **subpopulations) that are demographically connected via dispersal. 
...
But -- in fact, many metapopulation models ('classical metapopulations', covered in this lecture and Gotelli's Chapter 4) don't consider individuals at all! Instead, we only care about whether a subpopualation, or patch, is occupied (N > 0) or not (N = 0).  

## A 'classical' metapopulation?

In classical *metapopulation ecology*, we are generally considering a **landscape** with a certain number of potentially habitable **patches** of habitat. NOTE: we will use the term 'patch' and 'subpopulation' interchangeably in this lecture.

A classical metapopulation is simply a *population of patches*, each of which may or may not be *occupied* by a species of interest at a given point in time. Different sets of patches may be occupied at different points in time. 

![](MP2.jpg)

![](patch1.jpg)

To simplify, classical metapopulation models assume that all areas outside a patch are completely unsuitable (i.e., they are NOT habitat).   

We also often assume that all patches are similar (similar habitat quality, area and connectivity to neighboring patches), and therefore are more or less interchangeable. [NOTE: we revise this assumption when we start to consider 'source and sink' models below]   

Instead of keeping track of the individuals in each patch ($N$), we often just keep track of whether or not each patch is occupied (N > 0) or not (N = 0). Or even more simply, we just keep track of the fraction of occupied patches in our landscape ($f$)!

Instead of keeping track of the dispersal of individuals among patches, we only consider the *colonization* of unoccupied (empty) patches via immigration ($I$) and the *rescue effect* (immigration preventing extinction of a patch; see below). 
...
Instead of investigating population dynamics, we are investigating *metapopulation occupancy dynamics* -- that is, how the fraction of patches occupied changes over time.

Under the classical approach, it NOT a metapopulation unless some patches are unoccupied at some points in time!
