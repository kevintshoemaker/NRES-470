...
Broadly speaking, a **metapopulation** is a group of populations (often called **subpopulations**) that occupy spatially distinct habitat **patches** that are close enough to support movement of individuals among patches. 

Now that we're thinking about animals occupying habitat patches within a larger landscape (we are explicitly thinking about spatial locations for the first time in this class!), we need to incorporate information about **movement ecology** in addition to **population ecology**! 
...
Metapopulation ecology covers a broad range of scenarios: as long as we can conceive of subpopulations occupying distinct habitat patches in a landscape, and as long as there is some dispersal among patches, then we can model this set of populations through the lens of metapopulation ecology. 
...
The term "metapopulation model" is often used to refer to models where we don't keep track of the abundance in each patch ($N$ is no longer our primary unit of interest). Instead, we only consider **patch occupancy** (the number or fraction of patches in the metapopulation that are occupied) - like the models in Gotelli Chapter 4. That is, we only care about whether a patch is occupied (N > 0 in a given patch) or not (N = 0) each year.  

![](MP2.jpg){width=50%}

However, we CAN keep track of patch abundance in a metapopulation model if we really want to! In fact, if we want, each patch can contain a stage-structured, density dependent population (like the more complex metapopulation model in the final part of lab 6!).     
...
Classical metapopulaton models also often assume that all patches are similar (similar habitat quality, area and connectivity to neighboring patches), and therefore are more or less interchangeable. We will relax this assumption when we start to consider 'source and sink' models (below).   

The simplest classical metapopulation models (like those in Gotelli Ch 4) only keep track of the fraction of occupied patches in the landscape ($f$)!
...
In a classical metapopulation, the [Stock] we are studying now is NOT the total number of individuals but instead it is the fraction of patches occupied!
...
**Q**: What if the extinction of one patch was *perfectly correlated* with the extinction of all other patches. If one goes extinct, all go extinct. [Top Hat] 
