...
Birth rate, $b_t$, is the _per-capita rate of offspring production_ for age $t$. We can think of this as the number of offspring produced per female at the moment they reach $t$ years of age.   
...
Fecundity also takes into account the survival rate from $t$ to $t+1$!!  For an adult of stage $t$ to contribute to the next generation, _it must first survive_ before it can reproduce!.

$F_t = P_{t \rightarrow t+1} \cdot  b_{t+1}    \qquad \text{(Eq. 4)}$
...
2c. Build an InsightMaker model that represents the same population as the stage matrix above. Provide the URL to your InsightMaker model in your write-up (and remember to ensure that you don't alter the model once you submit it) .  
