...
Then we allowed $b$ and $d$ to become less favorable as population density increased- this resulted in logistic (S-shaped) population dynamics. 

Here we ask: what if population vital rates ($b$ and $d$) change as individuals get older (i.e., what if they are 'age-structured')? How can we model age-structured population vital rates? How might this affect population dynamics? 
...
The term **cohort** represents a bunch of individuals in a population that were all born at the same time. In a life table, the term S(x) refers to the number of individuals from a particular cohort that are _still alive at age x_. From this raw data, we compute two terms, called **survivorship** and **survival rate**

The term $l(x)$ represents the _proportion of the cohort surviving from age 0 to age x_. This is called **survivorship**

The term $g(x)$ represents the _proportion surviving from age x to age x+1_. This is called **survival rate** 
...
**Q**: can survivorship increase with age? [HINT: really think about the definition of survivorship!]
...
**Q:** Which survivorship curve is the most common in nature?? [PointSolutions]
...
What is a **generation**? The most common definition (for generation time) is the _Average age at which individuals in a cohort produce offspring_ (consider all the offspring produced by the cohort- what is the average age of their parents when they were born?). This can be computed from the life table as:
...
4. Load up an age-structured model in InsightMaker. You can clone [this one,  here](https://insightmaker.com/insight/70809/Age-structured-population). 
    ...
    4b. Can you modify the initial abundance of juveniles, subadults and adults so that the population exhibits smooth exponential growth for all three age classes (and the population as a whole)? This is called **Stable Age Distribution (S.A.D.)**
