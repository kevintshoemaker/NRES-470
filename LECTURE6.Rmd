...
When we began modeling wildlife populations, we treated $b$ and $d$ (per-capita birth rate and fractional death rate) as constants. This assumption led to unrestrained exponential growth.

Then we allowed $b$ and $d$ to change (becoming less favorable for growth) as population abundance/density increased- this resulted in logistic (S-shaped) population dynamics. 
...
Take this Indian elephant for example. How does the per-capita birth rate (b) depend on age for this species (i.e., what is the **birth schedule**?
...
What about per-capita death rates for a tortoise (the so-called *mortality schedule*)? We might imagine something that looks like this...

```{r echo=F}
Tortoise_age <- seq(0,120,by=5)

Death_rate <- c(seq(0.5,0.05,length=5),seq(0.02,0.02,length=10),seq(0.02,0.2,length=11))
...
**Q:** When is sex structure important to consider when modeling populations?
...
Age-structured populations are often represented by a table called a **life table**. A life table tracks a single **cohort** (group of organisms of the same species that are born during the same year) across time. 
...
The reproduction component of the life table is called the reproductive schedule (or sometimes, the "fecundity schedule" or "birth schedule")! The term $b(x)$ represents the per-capita birth rate for females of age x. The reproductive schedule describes how birth rate changes with age.   

### Survivorship schedule

Fecundity/reproduction is only half the story! The survival component of the life table is called the "survivorship schedule"! 

The term **cohort** represents a collection of individuals that were born into a population at the same time. 
...
The term $g(x)$ represents the _proportion of the cohort surviving from age x to age x+1_. This is called the **survival rate** (contrast this with survivorship!).
...
*Survivorship curves* describe how the logarithm of survivorship ($log(l(x))$) drops off with increasing age. 
...
In this exercise we will explore **age-structured populations**. 
...
BUT, what is the time frame of $R_0$? What is the time frame of $\lambda$? They are different- the difference is that $R_0$ describes growth per **generation** and lambda describes growth per time step. 

What is a **generation**? The most common definition (for generation time) is the _average age at which individuals in a cohort produce offspring_ (consider all the offspring produced by the cohort- what is the average age of their parents at their time of birth?). This can be computed from the life table as:

$$G = \frac{\sum_{x=0}^{k}l(x)\cdot b(x)\cdot x}{\sum_{x=0}^{k}l(x)\cdot b(x)}$$    
This represents the average age at which members of the cohort produce offspring. 
...
4. Load up a stage-structured model in InsightMaker. You can clone [this one,  here](https://insightmaker.com/insight/6vZsNuzGIbisYsFVCSLWZK/Stage-structured-population). 
    ...
    4b. Modify the initial abundance of juveniles, subadults and adults so that the population exhibits smooth exponential growth for all three stage classes (and the population as a whole)? This is called **Stable Stage Distribution (SSD)**
    
    4d. Now change the adult survival rate to 0.35. What happens? Is this a growing population? How many yearlings would need to be added to the population each year (population supplementation) to maintain a stable or growing population of >20 adults?  [Top Hat]

What do you think? Is supplementation the best strategy for conserving this population?       
