...
The reason? Populations grow in a positive feedback process. More individuals means greater potential for population growth (more reproduction!), which means more individuals enter the population, which means even greater potential for population growth. The bigger the population the faster the population grows. 
...
If we ignore immigration and emigration, then the BIDE equation simplifies to the "buh-duh" equation:

$\Delta N = B - D \qquad \text{(Eq. 2)}$

Now let's focus on $B$ and $D$. The important thing to recognize is that the total number of births and deaths in a population is not constant. 

What does the total number of births depend on? 

What **is** more likely to be constant is the **per capita rate** of births or deaths in a population. Does this make sense? These per capita rates are often expressed as lower case letters. So $b$ represents per capita births, and $d$ represents per capita deaths. 
...
## In-class activity: a basic population simulation model
...
4. Now click **Run Simulation**. Now you have run your first population simulation!
...
**Q**: what happens if you set the birth rate equal to the death rate? What is *r* for this population?

**Q**: what happens if you set the birth rate less than the death rate? What is *r* for this population?
...
These distributions are: the **binomial distribution**, the **Poisson distribution**, and the **Normal distribution**
...
3. Make sure that *Birth rate* is set to 0.5 and *Death rate* is set to 0.4. Set initial abundance to 10. Under the "Settings" menu set the model to run for 20 years. Also make sure your *Population* stock can NOT go negative (this is a setting in the configurations panel: Allow Negatives = "No").  
...
6. Change the initial abundance to 500 and re-run the "Sensitivity Testing" tool.


**Q** Is the effect of *demographic stochasticity* bigger at low or high abundances?

**Q** Is there any risk of extinction when the initial abundance is 10? What about when the initial abundance is 500?
...
3. Set initial abundance to 10. Under the "Settings" menu set the model to run for 20 years. Also make sure your *Population* stock can NOT go negative (this is a setting in the configurations panel: Allow Negatives = "No"). 
...
The first step (after you have formulated your research questions, of course!) is to conceptualize the key life history traits that you need to know to address your main questions. This can be done by visualized in a *life history diagram* (essentially and stock-and-flow diagram just like we have already been doing!). In general, the following life history traits are often critical to consider:

- Maximum and minimum reproductive age
- Number of relvant age classes
...
What *scenarios* do you want to test in order to answer your original questions?

### Step 5: Results

Finally, you need to make sense of all the simulations you just ran!  

There are two major tools that you will need to be able to use the simulation results to answer your questions: **graphical visualization** and **statistical analysis**. 

NOTE: The process of building a PVA model is **iterative** and **non-linear**. 

For example -- after running your model (step 4) you might realize that your results (step 5) are unrealistic. This might prompt you to go back and change your conceptual model of the life history (step 1), and re-parameterize your model (step 2).  
