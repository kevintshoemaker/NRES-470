...
In the first period we will talk through the basics of population modeling for wildlife conservation, and I'll give some examples of how this method is used in biological conservation.

In the second period, we will run through a case study involving the piping plover, where we will use a population model to address practical questions about the benefits of nest protection (predator exclosures placed around nests). 
...
Populations operate within an ecosystem, interacting with their environment (e.g., resources) and with other species (competitos, predators). These interactions result in positive and negative feedback loops, which in turn leads to large-scale **emergent properties** -- like the realized range of a species or the presence of an unstable population equilibrium (a tipping point) -- which are often difficult to predict based on the set of feedbacks considered individually! 

Mathematics gives us a language for unambiguously describing and modeling each functional relationship and feedback in the system. However, as systems get more complicated, closed-form mathematical solutions (e.g., finding mathematical expressions that 'solve' a set differential equations) become very difficult, and often impossible. That's where computers come in.   
...
Armed with skills in mathematics and computer programming, ecologists and natural resource professionals can *formalize their understanding* of the natural systems in which they work. That is, we can use formal models to account for complex ecological realities and that allow us to advance and test new theories and better understand these natural systems. 

In conservation biology, these formal models allow us to run virtual "experiments", or **scenario tests** using these models, asking questions that wouldn't otherwise be addressable. For example, to compare what the future might look like if we were to make a specific set of conservation or management decisions -- and to assess which strategies have the best chance of success! 

This is what PVA is all about.
