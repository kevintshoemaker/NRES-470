...
[Click here for Excel workbook link](NRES421_2026.xlsx)  

If you don't have Excel, try loading this file up using Google Drive and opening in Sheets.    


## Population Viability Analysis (PVA)

In this 2-period short course, we will explore the use of **population simulation models** to address conservation questions-- a process often called **population viability analysis (PVA)** 
...
In the first period we will talk through the basics of population modeling for wildlife conservation, and I'll give some examples of how this method is used in conservation biology.

In the second period, we will work through a case study involving the piping plover, where we will use a population model to address practical questions about the benefits of nest protection (predator exclosures placed around nests). 

Let's review some population ecology theory first.
...
Defining a population is not always straightforward. For example, spatial boundaries defining populations sometimes are easily defined, but other times are vague and difficult to determine. Where does one population begin and another one end?
...
The following figure illustrates the growth of a population starting from 10 individuals and with an annual growth rate of 15% per year.

```{r echo=FALSE}

options(scipen=10)
expf <- function(x){10*exp(0.15*x)}
curve(expf,0,100,xlab="years",ylab="Abundance (N)",lwd=3,col=gray(0.4))

text(20,30000000, "initial abundance = 10")
text(25,20000000, "intrinsic rate of growth = 0.15")

```


![](Human_population_growth_from_1800_to_2000.png){width=40%}

![](positivefeedback1.png){width=40%}

The takeaway? With sufficient resources (e.g., plenty of food, shelter, nest sites, etc.), collections of living things grow in a **positive feedback** loop. More individuals in the population means greater potential for growth (e.g., more babies entering the population!), which means EVEN GREATER potential for population growth.  

## Systems thinking, mathematics and computer modeling 

Populations operate within an ecosystem, interacting with their environment (e.g., resources) and with other species (competitors, predators). These interactions result in positive and negative feedback loops, which in turn leads to large-scale **emergent properties** -- like the realized range of a species or the presence of an unstable population equilibrium (a tipping point) -- which are often difficult to predict based on the components of the model considered individually! 

Mathematics gives us a language for unambiguously describing and modeling each functional relationship and feedback in the system -- often via a system of difference (or differential) equations. However, as systems get more complicated, closed-form mathematical solutions (e.g., finding mathematical expressions that exactly 'solve' a system of differential equations) become very difficult, and often impossible. That's where computers come in.   

Computers have reduced or eliminated many of the barriers to modeling highly complex systems and understanding how these complex systems behave. As a result **computer programming** is a critical component to modern ecosystem studies and conservation biology, which is fundamentally concerned with understanding and managing complex systems. 

Armed with skills in ecology, mathematics and computer programming, ecologists and natural resource professionals can *formalize their understanding* of their study systems. That is, we can assemble formal mathematical representations of complex ecosystems one component at a time, based on our understanding of how the individual parts work and interact. And by putting the pieces together into a single framework, we can understand the implications of the interactions and feedbacks-- allowing us to advance new theories, test existing theories, and better understand how complex natural systems function. 

In conservation biology, these formal models allow us to run virtual "experiments", or **scenario tests** using these models, asking questions that we wouldn't otherwise be able to address. For example, to compare what the future might look like if we were to make a specific set of conservation or management decisions -- and to assess which strategies have the best chance of success! 

This is what PVA is all about. It is the applied side of ecosystem and population modeling- where the rarefied world of quantitative ecology escapes the walls of the ivory tower to impact natural resources policy decisions in the real world!  

## Modeling exponential growth!

Now let's assume that *per-capita* birth and death rates in the population are constant across time, such that per-capita birth rates exceed death rates by a certain fixed constant (known as the intrinsic rate of growth, $r$. If the population gets bigger, more net individuals will be added to the population in the next time step, making the population even bigger! This is a positive feedback! 

Open up the first workbook in our Excel spreadsheet. Take a minute to play around with the green-shaded cells (under "your scenario": birth rate, death rate initial abundance) and see how the growth of the population size (y axis) changes over time (x axis) in the embedded plot. 

Look at how the following two derived parameters change as you change the three parameters: intrinsic growth rate (r) and Lambda. 

Intrinsic rate of growth is simply the per-capita birth rate minus the per-capita death rate.

Lambda is the multiplier that converts this years abundance to next years abundance ($N_{t+1} = N_t\lambda$).

### Quick exercise

Can you come up with the 'rules' of population growth relative to r and lambda? What values of r and lambda correspond to a growing population? What values of r and lambda correspond to a declining population? 

## An even more realistic model!!

Next, let's add some negative feedback process that slows population growth as the population gets larger. For example, see the second worksheet in our Excel workbook. 

We won't do any exercise, but take a minute to play around with this slightly more realistic model- again, only altering the green-highlighted cells. 

We could keep on adding more and more realism and complexity, but hopefully you get the picture!

## Case study: Gopher tortoise conservation status

[PPT slides](GT1.pptx)

### Quick exercise

Before moving on, pick any parameter and try varying the parameter from the minimum to the maximum value indicated in the spreadsheet (only change the cells highlighted in green). 

Based on the results of your tests: (1) was the population growth rate sensitive to this parameter- and was the effect greatest in the original model or the corrected model? (2) what are the implications for the conservation of gopher tortoises??

## Case study: Red Knots and Horseshoe Crabs

[PPT slides](REKN2.pptx)

### Quick exercise

Before moving on, try changing the abundance of horseshoe crabs using the guidelines in the Excel spreadsheet -- that is, try running the model with "average" horseshoe crab abundance, "highest observed" abundance, "lowest observed" abundance, and a scenario of a collapsed horseshoe crab population. 

How do the two models (original and reanalysis) differ in terms of the sensitivity of the population growth to the abundance of horseshoe crabs? If you were involved in managing the harvest of horseshoe crabs in Delaware bay, what harvest guidelines would you recommend when presented with these alternative models of the system? 

## Case study: Piping Plover nest protection

[Plover slides](Plover1.pptx)
...
Any **dynamical system** (like a wildlife population) consists of [Stocks] (the quantities of interest) and [Flows] (the inputs and outputs to these quantities).

A [Stock] represents a collection of something- like individuals in a population. 

The [Stock] changes over time ONLY via [Flows In] or [Flows Out]. 

Imagine our stock represents *Moose*! 

*Moose Births* would then be the input [Flow] to the population of *Moose* over time (number entering each year). Similarly, *Moose Deaths* would represent the output [Flow] (number exiting each year).
...
This next demonstration model is wrong, and not useful! But it's a start!

### Simple stock and flow model! 

Open up [InsightMaker](https://insightmaker.com/). If you have never used it before you need to sign up first with a username and password. InsightMaker is free and web-based.

Click on [this link](https://insightmaker.com/insight/162300/NRES-421-In-Class-Demo-1) to load up the first population modeling demo.
...
Click on [this link](https://insightmaker.com/insight/162299/NRES-421-In-Class-Demo-2) to load up the next population modeling demonstration.
...
One of the most fundamental quantities in population ecology is called the "intrinsic rate of growth", or *r*. R is simply the difference between the per-capita birth and death rate:
...
Next, we have several options. We might decide that the next thing we would add to make this model more realistic is some negative feedback process that slows population growth as the population gets larger. For example, something like this:

https://insightmaker.com/insight/5iMKBqMtkz5e2WcJM3YQWN/Logistic-Growth-1-NRES-421-621

We could keep on adding more and more realism and complexity, but hopefully you get the picture!


## More realism: stochastic models
...
# 
# **Short Answer Question**: What is the extinction risk for this population? Is this extinction risk *acceptable*? If not, what is an acceptable risk of extinction?
# 
...
# 
# **Short Answer Question** Is the effect of randomness more important/pronounced at low or high abundances? Is there any risk of extinction when the initial abundance is 10? What about when the initial abundance is 500?
# 
# [pause and record your answers on WebCampus]


# [Red Knot model](https://insightmaker.com/insight/hoWpE5JlD9x5AaPHujVg4/Red-Knot-Matrix-Model)
# 
# [Horseshoe Crab model](https://insightmaker.com/insight/gim35Z9XDbmFy7gcQy2JD/Horseshoe-crab-model)
# 
# [Key PPT slides for exercises](REKN.pptx)


# [--go to grizzly PVA!--](PVA2_421.html)

# We will use [InsightMaker- a free web-based systems modeling tool](https://insightmaker.com/) for running hands-on simulation models. 

# the use of population models to support the conservation and management of the *rufa* Red Knot (*Calidris canutus rufa*) and the Horseshoe Crab (*Limulus polyphemus*), whose eggs provide a key source of food for migrating Red Knots.
...
# [Horseshoe Crab model](https://insightmaker.com/insight/gim35Z9XDbmFy7gcQy2JD/Horseshoe-crab-model)
# 
# [Key PPT slides for exercises](REKN.html)
