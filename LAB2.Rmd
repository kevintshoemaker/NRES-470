...
Using some algebra (see the Gotelli book), we can simplify this to:
...
6. Set initial abundance equal to carrying capacity (K). _Since the abundance is now initialized at carrying capacity (K), abundance should not change over time_ when you run the model. 
...
2g (image upload). Using equation 6 as a starting point, analytically solve for **Maximum Sustainable Yield** (maximum number of individuals that can be harvested sustainably each time step)? Upload an image file to WebCampus showing your calculations (you can use a drawing app, a "web whiteboard" application, or upload a photo of your work).

HINT: $r_{max}$ (maximum per capita growth rate) is the difference between the maximum birth rate and the minimum death rate! 

HINT: under the logistic growth model, the population has its highest growth potential (and therefore the highest potential sustainable yield) at exactly one-half of K [e.g., K/2]). 

HINT: at MSY, the net number of new individuals produced by the population per time step (i.e., $\Delta N$) is equal to the number harvested (this is by definition- at any equilibrium point, the inputs flows are equal to the output flows). 

NOTE: Your analytical solution for MSY (maximum sustainable harvest rate) should closely match the answer you found by trial-and-error in 2a. If it doesn't one or both of your above models is wrong- that means you should go back and try again!


## Exercise 3: basic logistic growth model (single stock, single flow)
...
## OPTIONAL: Exercise 4: delayed density-dependence! [optional- not part of the 'official' lab and does not count towards your Lab 2 grade]
