...
Imagine a social species, like the prairie dogs pictured above. If the density of prairie dogs is high, they can warn each other about predators. If the density is low, there may not be enough neighbors around to warn each other of approaching danger, and the predation rate might go up, resulting in higher *d*. This is a positive feedback, and can result in, well, you should simulate it for yourself! 
...
Passenger pigeons were an extremely gregarious species. Once their numbers dwindled (due to hunting and clear-cutting) one hypothesis is that their social systems broke down, and they could no longer effectively reproduce or protect themselves from predators.   
...
2. Now add a new [Variable] in InsightMaker called *Allee threshold*. Set this variable equal to 200 (it is a 'constant' and will remain at this value through the simulation).

3. As for the other parameters, set them as follows:    

- Initial abundance: 201 (just above the Allee threshold) 
- Density dependence on birth rate: 0.0004    
- Density dependence on mortality: 0.0001   
- Max birth rate: 0.8   
- Min mortality: 0.3   
...
5. If we are running out of time in class (which we probably will be!), load the model (and clone it!) using [this link](https://insightmaker.com/insight/3iPYWl5vT2niZ1WGtC7C7Y/Allee-effect-lecture-5)!    

Now you can try to answer the following questions:

**Q**: Carrying capacity (K) represents one equilibrium point in this model. Try to find another equilibrium point- that is, a point where the population neither grows nor declines.  [PointSolutions] 

**Q**: is this equilibrium a *stable equilibrium* or an *unstable equilibrium*? [PointSolutions]

Hopefully it is clear from this example why Allee effects often spell bad news for wildlife conservation!  
