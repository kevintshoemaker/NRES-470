...
Another continuous distribution you should know is called the *Log-normal distribution*. This distribution has a lower bound of zero and an upper bound of $\infty$.

**Q** What population parameter might the log-normal distribution be useful for modeling?
...
**Q** What population parameter might the beta distribution be useful for modeling?
...
Can you change one of the above functions to produce more than 1 random number?

Try to change one of the other parameters and generate 100 random numbers? To visualize the distribution you can use the "hist()" function to produce a histogram of the random distribution you just generated!    
...
Now let's try to fit a distribution to these data! We could use a normal distribution (which would be perfectly fine), but because the total per-capita number of eggs successfully hatched cannot go below zero (but has no defined upper bound) it's even more appropriate to use a lognormal distribution!
