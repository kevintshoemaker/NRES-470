...
**when and where** The first midterm exam (out of two) is coming up Wednesday **March 11**. You will have the whole 50 minute class period to take the exam. The exam will be on paper this year. 

**what** The exam will cover:

- Chapters 1-3 of the Gotelli book *including matrix population modeling concepts*. Note that any material in the Gotelli book that we did NOT cover in class will NOT be on the exam. 

- All material covered in lectures, including the following webpages:           
...
- Basic programming concepts:
    ...
    * NOTE: you will NOT be tested on how to construct and run population models in R (or how to do anything else in R).

*The exam will consist of a mixture of multiple-choice and short-answer questions*      

*You do NOT need a calculator. Even if I ask a math question you will get full credit for setting the problem up correctly- the exact answer is not important, since the exam is meant to test understanding of fundamental concepts in population ecology*.   

We will hold a review session for the exam during our regular class meeting on the Monday prior to the exam. Please make note of any questions that come up as you study- we can talk these over during the review session!

![](uncertainty2.jpg){width=20%}
...
- **The system is inherently variable and unpredictable**. In population ecology this is also known as **demographic stochasticity** and **environmental stochasticity**. *In practical terms: no matter how much we study and understand a natural system, we can not predict the future with certainty*.

Ecological systems are inherently variable and unpredictable. We can't know with certainty whether or not an individual will mate, or die. We can't really even be certain whether the environmental conditions (e.g., moisture availability, food availability) will be favorable or unfavorable for offspring production or survival! 
...
### Uncertainty analysis (dealing with lack of knowdledge)

What if we simply don't have enough data to build a perfect model? This is an example of *lacking sufficient knowledge*. In this case, the tool we use is called **uncertainty analysis**. Basically, we run the model using "worst case" and "best case" values for a parameter (usually a per-capita vital rate- often modeled as [Variables] in InsightMaker) we are uncertain about (e.g., adult survival rate could be 0.6 or 0.7) and see what happens to our study system (e.g., given the uncertainty we have about the true parameter value, is it plausible that the population will go extinct? Is it plausible that it will decline?). In this class, we refer to this as the "best-case/worst-case" method.

### Stochastic models (demographic and environmental stochasticity)

Here, we embrace the concept that populations are inherently variable and unpredictable.

1. We usually can't predict whether an individual will live or die, breed or not breed, etc. All we can know is the *probability* of mating, or the *probability* of dying, or the per-capita *rate* of offspring production, or the probability of a given offspring being female. But when it comes to projecting exactly which individuals live or die (whether an individual will be "lucky" so to speak!), who gives birth and who doesn't, and how many females are born, we _just can't know for sure_. This is an example of a system being inherently unpredictable. In population ecology, this is called **demographic stochasticity**. In this case, we **randomly sample total births and deaths** (the [Flows] in InsightMaker) to represent the inherent unpredictability of individual fates within a population.

2. We usually can't predict whether next year or the year after will be favorable or unfavorable for population growth- that is, whether the per-capita vital rates (e.g., $b$, or $d$ or $r$) will be more or less optimal at a future point in time (whether a _population_ will be "lucky", so to speak). This is another example of a system being *inherently unpredictable*. In population ecology this is called **environmental stochasticity**. In this case, we **randomly sample per-capita vital rates** (represented as [Variables] in InsightMaker) to represent the inherent unpredictability of environmental conditions. 

![](dice1.png){width=15%}

## Random number generation
To incorporate inherent unpredictability (randomness) into our models, we often include *stochastic* components so that each model run (replicate) -- or each year of each model run -- is different from one another (the results are variable). That is, we need to include at least one **random number generator** in our models! A random number generator is like a lottery machine! Each time we want a new number we reach in and pull another number out, record the number, put it back in and shake it up again.
...
**Top Hat** In R, generate one random number from a Binomial distribution with 10 trials and probability of success equal to 0.3

In R, generate random numbers from a Poisson distribution with mean of 2.1

In R, generate random numbers from a Normal distribution with mean of 1.75 and standard deviation of 0.9 
...
**Q**: What type of uncertainty does this set of values most likely represent? What random number generator might you use to represent this uncertainty? [Top Hat]
...
**Q**: what is the best-fit value for "mean"?  [Top Hat]               
**Q**: what is the best-fit value for "sd"?  [Top Hat]                        
...
You can clone this one if you'd like: [https://insightmaker.com/insight/2WNEa6ZT3Z23CfLbuQqfJR/base-exp-growth-b-and-d](https://insightmaker.com/insight/2WNEa6ZT3Z23CfLbuQqfJR/base-exp-growth-b-and-d)
...
[InsightMaker Demo](https://insightmaker.com/insight/5ERWK9uvmRCuuvvbWOHpJL/stoch-demo-3-30-26)
