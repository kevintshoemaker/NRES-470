...
## The course motto: *Be wrong (but useful)*! 
...
Finally, a [Variable] is an object that stores an important piece of information for your model (e.g., birth rate could be a [Variable] that informs the total [Flow] of individuals into a [Stock] representing a wild population). [Variables] are generally connected to [Flows] with [Links] -- unlike [Flows], links do not represent any physical process. Instead, links simply represent a transfer of *information*.   
...
4. **Specify the births process** Select the [Flows/Transitions] button in the upper left corner of your screen. Now notice that when you mouse over the [Stock] a small right-arrow displays. Click on the right-arrow and drag a couple inches to the left of the [Stock] and release. Type in a name for what this arrow represents (Births). NOTE: you have actually just created a [Flow] *out* of a [Stock]. To create a [Flow] *into* a [Stock], click the **Reverse** button in the top menu (looks like two opposing arrows). Please do that now. Also in the **Configuration Panel** set **Flow Rate** = 1.
...
*Take a few minutes to list the things you would like to change to make this model more realistic!*
