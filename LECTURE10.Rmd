...
So far in this class we have considered the **population** as the fundamental unit that we are interested in modeling. The population is a [Stock] and all processes that can change the size of the population are [Flows]. All individuals within any [Stock] are assumed to be interchangeable (identical in every way).  
...
In some ways a population is hard to define- it can be a bit of an artificial concept. Where exactly does a population begin and end? 
...
**Population-based models** (PBM; i.e., models of $N$) are the main type of model we consider in this class- because it is the most simple and direct way to think about population dynamics: with all individuals in a [Stock] considered to be interchangeable, $N$ is our main variable of interest. $N$ is in turn controlled by endogenous (forces coming from inside the population -- e.g., density-dependence, demographic stochasticity) and exogenous (forces coming from outside the population -- e.g., environmental stochasticity, harvest) factors. 

**Age/stage structured population-based models** and **sex structured population models** (i.e., models of $\mathbf{N}$, i.e., matrix population models) are a type of population model in which individuals are grouped together according to important traits like sex and age, with distinct population vital rates assigned to each group.
...
In this case, we don't model $N$ directly at all -- in fact, $N$ (in an IBM framework) is an **emergent property** of individual organisms interacting with each other, with predators and competitors, and with their abiotic environment.
...
The locals now realize: to restore ecological balance to the islands *They need to reintroduce wolves*!

The local natural resources management agency is about to start an experimental wolf reintroduction in a very small island in the archipelago (as a test), but they want to know how to proceed.  
...
- How many wolves should be introduced and over what period of time to produce the desired ecological effect (lush carpets of green grass)?

They give you some information to get a first guess and they promise that you’ll be able to come study this natural system once the reintroductions are underway.
...
- Each sheep eats 1 "unit"" of grass per day (see section on grass, below)
- Each sheep gives birth to 2 lambs approximately every 100 days of the experiment (very high fecundity!).
...
- Wolves can kill a maximum of one sheep per day.
