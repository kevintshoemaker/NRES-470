...
Another way to do *uncertainty analysis* is to use a random number generator to represent uncertainty. Specifically, we can use a *uniform distribution* to represent the range of uncertainty about the parameter value. In this case, we can define Birth rate as a **uniformly distributed** random variable with minimum of 0.15 and maximum of 0.35 (so that the random number generator produces values within this range). We can do this in InsightMaker in one of two ways: one way is to define birth rate like this (within the 'birth rate' variable):
