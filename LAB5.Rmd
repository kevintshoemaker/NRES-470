...
NOTE: in this lab we will use the "worst-case/best-case" method for modeling insufficient knowledge. We will also explore another commonly used method that uses a random number generator! 

## Inherent unpredictability of population outcomes

Even when we have plenty of data, there will be substantial uncertainty about the future! This uncertainty is derived from two sources:
...
In these cases, we can use **random number generators** to embrace the inherent unpredictability in our study systems in our population models. 
...
We embrace this form of uncertainty by making the annual total number of births and deaths **stochastic** (random). That is, we use a *random-number generator* to generate realistic variation in the total births and total deaths (the Flows in our InsightMaker models).  

#### Demographic stochasticity in births

For the total number of births in a population each year (the 'Births' flow in InsightMaker), we usually use a **Poisson distribution**. The Poisson random number generator only produces whole numbers (we don't like fractional individuals anyway) and doesn't produce negative numbers. This makes it a good way to model births! 
...
Set *Birth rate* equal to 0.16 and *Death rate* equal to 0.15. Set *initial abundance* to 10. Under the "Settings" menu set the model to run for 50 years. It's generally good practice to make sure your *Population* stock can NOT go negative (this is a setting in the configurations panel). Hit "Simulate"- you should see positive *exponential growth*! 

What if we have imperfect knowledge about birth rate? The data we have are consistent with a birth rate $b$ as low as 0.14 and as high as 0.18. Run the model with the lowest and the highest possible birth rates. Now use the "Compare Results" tool (under the "Tools" menu in the upper right corner...) to visualize the range of possible population growth trajectories that would be possible given our *uncertainty* about the per-capita birth rate (i.e., plot the "worst-case scenario" and the "best-case scenario" in the same plot). The "truth" about the future abundance of this population could be anywhere in between the lower and the upper bound!
...
1b (short answer). Given the uncertainty we have about the birth rate parameter $b$, what is the lower and upper bound (worst case and best-case) for the final abundance at year 50? Would it be reasonable to conclude that this is a stable or growing population (lambda > 1)? Please justify your response.  

Now, let's use a random number generator to represent uncertainty about the birth rate parameter instead of using the "best-case/worst-case" approach (like we did in 1a, 1b). 

Here we will use a *uniform probability distribution* to represent our uncertainty about the birth rate parameter. 

In InsightMaker, change the "Birth rate" parameter $b$ (previously set at 0.16) to instead draw from a **random uniform distribution** with minimum of 0.14 and maximum of 0.18 (so that the random number generator always produces values within this range). 
...
First, make sure to set *Birth rate* in your model back to 0.16. Make sure initial abundance is set at 10 individuals and *Death rate* is set at 0.15. Hit "Simulate"- make sure you still see exponential growth!     
...
NOTE: you can use the "random number functions" menu in the equation editor to have InsightMaker give you the proper syntax. Try to avoid relying on typing commands directly in InsightMaker.
...
For this exercise, we will assume that the annual birth rate $b$ in this population has a mean of 0.16 but varies year to year with a standard deviation of 0.11. 

To do this in InsightMaker, you can use the following formula for the *Birth Rate* variable:

```
RandNormal(0.16, 0.11)
```

For the death rate $d$, we will assume that the annual birth rate in this population has a mean of 0.15 but varies year to year with a standard deviation of 0.1. 
...
> MVP: The initial abundance below which the risk of extinction equals or exceeds 5% over the next 50 years.

Another way to think about MVP is the initial abundance threshold above which the risk of extinction is deemed acceptable (in this case, < 5%)

Here are the model parameters! (note that much of this is simplified and/or made up entirely!)

- The starting abundance is 200 individuals (females).
- No age/stage structure!
- Mean per-capita birth rate $b$ is 0.79 when the population is below carrying capacity (per-capita birth rate)
- Mean per-capita death rate $d$ is 0.71 (per-capita death rate)
- Density dependence:   
    - Carrying capacity is 250 individuals (based on estimates of remaining habitat availability).
    - Above carrying capacity, per-capita birth rate drops to 0.3 (for simplicity, you can assume no environmental stochasticity when the population is above K)
- Environmental stochasticity:   
    - when the population is below carrying capacity, birth rate is normally distributed with a mean of 0.79 and standard deviation of 0.25
    - when the population is above carrying capacity, birth rate is 0.3 with no variability across years
    - death rate does not change year to year (it stays constant at 0.71 per year).
...
NOTE: to avoid errors in your sensitivity test, you may have to prevent the birth rate from ever dropping below zero (producing negative numbers). To do this, use the following syntax for the "Births" flow:
...
3e (short answer). The Aruba island rattlesnake is limited to the small island of Aruba. This fact alone is one of the main reasons this species is thought to be highly endangered*. Why is small range size strongly associated with high extinction risk? [HINT: there is no one right answer- we are just looking for thoughtful responses]


## Checklist for Lab 5 completion

* Please submit your responses _on WebCampus_! See WebCampus for due date.
