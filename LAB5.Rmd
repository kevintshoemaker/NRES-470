...
NOTE: in this lab we will use the "worst-case and best-case" method for modeling insufficient knowledge. We will also explore another relatively commonly used method that uses a random number generator! 
...
The total number of deaths or survivors in a population each year (the Deaths flows and Transition flows in InsightMaker) are best modeled using a **Binomial distribution**. 

The Binomial random number generator only produces whole numbers (which we like- no fractional individuals!) and can only produce numbers that are greater than or equal to 0 and less than or equal to the total population size. These properties make it useful for estimating the total mortality or survival events each year!
...
NOTE: you can use the "random number generators" menu in the equation editor to have InsightMaker give you the proper syntax. In general, try to avoid relying on typing commands directly in InsightMaker. Also, the "Fix" function can be found in the "Historical Functions" menu. 
