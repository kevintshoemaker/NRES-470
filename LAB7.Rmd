...
Next, since we haven't had the lecture on this topic yet, let's go over the basics from the lecture first- [click here](LECTURE15.html)
...
If may not be as easy to see, but if we didn't account for imperfect detection, our estimates of key population parameters like survival and fecundity/recruitment would be highly *biased* in most cases. 
...
$\hat N = \frac{R}{M}\cdot N + (1-\frac{R}{M})\cdot N $

$M=\frac{R}{M}\cdot N = p \cdot N $
...
$R_t$ = Number of individuals already marked (Recaptures) when caught in visit $t$ (the remainder of individuals captured in visit *t* must be given a new mark)
...
NOTE: Program MARK is a Windows-based software. If you have a Mac, you can run MARK using a Windows partition or a Windows emulator. However, you don't need to use Program MARK for this lab exercise -- you can use R instead!

The 'mra' package in R runs many of the simpler open-population models, and is much easier to use than Program MARK. Please install this package if haven't already done so.
...
An "INP" file can be downloaded [here](ed_males.inp). If this doesn't work, you can save this [text file](ed_males.txt) and re-name it with the extension ".inp" rather than the extension ".txt"

NOTE: if this doesn't work, you can load the example files from the ["gentle introduction" link](http://www.phidot.org/software/mark/docs/book/)! 
...
dipper.male <- dipper[which(dipper$sex=="Male"),]   # extract only the males, to compare with Mark
...
allmodels <- crm.wrapper(cml,data=dipper.proc, ddl=dipper.ddl,external=FALSE,accumulate=FALSE)
